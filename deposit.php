...
// Get account info
$stmt = $pdo->prepare("SELECT a.id, a.balance, a.account_number FROM accounts a WHERE a.user_id = ?");
$stmt->execute([$user_id]);
$account = $stmt->fetch();
...
$pending_count = 0;
foreach ($deposit_requests as $req) { if ($req['status'] == 'pending') $pending_count++; }
?>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert-info { background: #e6f0f7; color: #0f5c8c; }
        .requests-section {
            background: rgba(255, 255, 255, 0.95);
            backdrop-filter: blur(10px);
            border-radius: 30px;
        ...
        .request-amount { font-size: 1.5rem; font-weight: 700; color: #0f5c8c; }
    ...
    <div class="container">
        ...
        <div class="glass-card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group"><label>Deposit Amount (<?php echo $currency_symbol; ?>)</label><input type="number" name="amount" step="0.01" min="10" placeholder="Enter amount" required></div>
                ...
                <div class="form-group"><label>Reason for Deposit</label><textarea name="reason" rows="3" placeholder="e.g., Savings, Bill Payment, Investment, etc." required></textarea></div>
                <div class="form-group"><label>Upload Payment Receipt (Optional)</label>
                    <div class="upload-area" onclick="document.getElementById('receipt').click()">
                        <i class="fas fa-cloud-upload-alt"></i>
                        <p>Click to upload receipt (JPG, PNG, PDF)</p>
                        <small>Max 5MB</small>
                    </div>
        ...
        <div class="requests-section">
            <h3><i class="fas fa-history"></i> My Deposit Requests <?php if ($pending_count > 0): ?><span class="status-badge" style="background: #fef08a; color: #854d0e;"><?php echo $pending_count; ?> pending</span><?php endif; ?></h3>
            ...
            <?php else: ?>
                ...
                <div class="request-item">
                    ...
                    <?php if ($req['status'] == 'approved'): ?>
                        <div style="margin-top: 15px; padding: 10px; background: #dcfce7; border-radius: 8px; color: #166534;"><i class="fas fa-check-circle"></i> ✅ Amount has been added to your account balance!</div>
                    <?php elseif ($req['status'] == 'rejected'): ?>
                        <div style="margin-top: 15px; padding: 10px; background: #fee2e2; border-radius: 8px; color: #991b1b;"><i class="fas fa-times-circle"></i> ❌ Deposit request was rejected.</div>
                    <?php else: ?>
                        <div style="margin-top: 15px; padding: 10px; background: #fef08a; border-radius: 8px; color: #854d0e;"><i class="fas fa-clock"></i> ⏳ Awaiting admin approval.</div>
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <script>
        document.getElementById('receipt').addEventListener('change', function(e) {
            if (e.target.files.length > 0) {
                document.querySelector('.upload-area').innerHTML = '<i class="fas fa-check-circle" style="color: #10b981;"></i><p>File selected: ' + e.target.files[0].name + '</p><small>Click to change</small>';
            }
        });
    </script>
</body>
